...
export default function Home() {
	return (
		<main className="min-h-screen bg-[#f6f4ee] font-sans text-[#202722]">
			...
			<section className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20" id="departamentos"><div className="mb-8"><p className="mb-2 text-[11px] font-bold uppercase tracking-[.13em] text-[#385347]">La colección Moreno</p><h2 className="font-serif text-[32px] font-normal tracking-[-.035em] text-[#385347] md:text-[40px]">Todos los departamentos</h2><p className="mt-2 text-sm text-[#68716a]">{departamentos.length} espacios disponibles para tu estadía</p></div><div className="grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-6 lg:grid-cols-3">{departamentos.map((apartment) => <TarjetaDepartamento departamento={apartment} key={apartment.name} />)}</div></section>
		</main>
	);
}
